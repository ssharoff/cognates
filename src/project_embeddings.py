...
def main():
    ...
    for line in f:
        ...
        if len(x)==2:
            ...
            print('WARNING: Error in line %s' % line, file=sys.stderr)
        try:
    ...
    l1=args.src_embeddings.rfind('/')+1
    l2=args.trg_embeddings.rfind('/')+1
    print("Printing the translation matrix to %s-%s.trn" % (args.src_embeddings[l1:l1+2], args.trg_embeddings[l2:l2+2]), file=sys.stderr)
    np.savetxt("%s-%s.trn" % (args.src_embeddings[l1:l1+2], args.trg_embeddings[l2:l2+2]), w)
