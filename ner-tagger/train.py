...
optparser.add_option(    "-o", "--wordlist", default="",    help="Other words location")
optparser.add_option(    "-s", "--tag_scheme", default="iobes",    help="Tagging scheme (IOB or IOBES)")
optparser.add_option(    "-l", "--lower", default="0",    type='int', help="Lowercase words (this will not affect character inputs)")
...
optparser.add_option(    "-p", "--pre_emb", default="",    help="Location of pretrained embeddings")
optparser.add_option(    "-A", "--all_emb", default="0",    type='int', help="Load all embeddings")
...
optparser.add_option(    "-D", "--dropout", default="0.5",    type='float', help="Droupout on the input (0 = no dropout)")
optparser.add_option(    "-L", "--lr_method", default="sgd-lr_.005",    help="Learning method (SGD, Adadelta, Adam..)")
...
assert os.path.isfile(opts.wordlist)
...
test_wl = loader.load_sentences(opts.wordlist, lower, zeros)
...
if parameters['pre_emb']:
    ...
    dico_words, word_to_id, id_to_word = augment_with_pretrained(
        ...
        list(itertools.chain.from_iterable(
            [[w[0] for w in s] for s in dev_sentences + test_sentences + test_wl])
...
prepare_dataset(
    test_wl, word_to_id, char_to_id, tag_to_id, lower
)
